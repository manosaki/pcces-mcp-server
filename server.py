    ...
    return [
        ...
        types.Tool(
            ...
            description=(
                ...
                "【呼叫前的引導原則】若使用者描述的資源模糊，請先詢問：\n"
                "1. 資源類型：人力（L）／機具（E）／材料（M）\n"
                "2. 人力：職稱全名、年資等級（無年資／初級／中級／高級）、計費單位（時／日／月）\n"
                "3. 機具：噸數／容量等規格、計費單位（時／日）\n"
                "4. 材料：規格、單位\n"
                "確認後以精確描述詞搜尋。"
            ),
            inputSchema={
        ...
        types.Tool(
            ...
            description=(
                ...
                "直接查詢全國標準定義，不依賴專案資料，任何安裝了 PCCES 的電腦均可使用。"
                "回傳 pccesCode（10碼）、工項名稱、單位。\n"
                "【呼叫前的引導原則】若使用者的關鍵字模糊（例如：混凝土、模板、油漆、防水），"
                "請先依序詢問以下資訊以取得精確關鍵字，不要直接用模糊詞搜尋：\n"
                "1. 工程類別：建築／道路橋樑／水利／景觀／其他\n"
                "2. 具體用途或部位：柱／樑／地板／外牆／天花板／屋頂…\n"
                "3. 材質或規格：厚度／強度／耐燃等級／表面處理…\n"
                "確認後以完整描述詞（如「鋼筋混凝土柱」「明架矽酸鈣板天花板」）搜尋，"
                "結果更精確且速度更快。"
            ),
            inputSchema={
                "type": "object",
